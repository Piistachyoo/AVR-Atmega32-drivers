...
#include "../LCD_8bit/lcd_interface.h"
...
void LCD_8bit_vSendString_pos(const LCD_8bit_cfg *PTR_LCD, u8 *Copy_u8String, u8 row, u8 column){
	LCD_8bit_SetCursorPosition(PTR_LCD, column, row);
	LCD_8bit_vSendString(PTR_LCD, Copy_u8String);
}
...
void LCD_8bit_SetCursorPosition(const LCD_8bit_cfg *PTR_LCD, u8 column, u8 row){
	column--;
		switch(row){
			case ROW1: LCD_8bit_vSendCmd(PTR_LCD, (ROW1_Address + column)); break;
			case ROW2: LCD_8bit_vSendCmd(PTR_LCD, (ROW2_Address + column)); break;
